...
def draw_barred_line(dwg, x1, y1, x2, y2, shorten_by=20, color="black"):
    ...
    line = dwg.add(
        dwg.line(
            start=(new_x1, new_y1), end=(new_x2, new_y2), stroke=color, stroke_width=1.5
        )
    )
    ...
    dwg.add(
        dwg.line(
            start=(bar_start_x1, bar_start_y1),
            end=(bar_start_x2, bar_start_y2),
            stroke=color,
            stroke_width=1.5,
        )
    )
    ...
    dwg.add(
        dwg.line(
            start=(bar_end_x1, bar_end_y1),
            end=(bar_end_x2, bar_end_y2),
            stroke=color,
            stroke_width=1.5,
        )
    )
...
draw_human_svg(
    dwg, center_x, center_y - vertical_offset, circle_dict, body_path, color="blue"
)
...
for i in range(n):
    ...
    draw_human_svg(dwg, x, y, circle_dict, body_path, color="grey")

    line, start_x, start_y, end_x, end_y, length = draw_barred_line(
        dwg, center_x, center_y, x, y, shorten_by=20
    )

    lines_info.append((line, start_x, start_y, end_x, end_y, length))

    if length < shortest_distance:
        shortest_distance = length
        shortest_line_info = (line, start_x, start_y, end_x, end_y, length)
